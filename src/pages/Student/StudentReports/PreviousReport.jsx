import { Card, Input, Form, Table, Button, Space, Divider } from "antd";
...
const PreviousReport = ({studentUserId}) => {
  ...
  const fetchStudentResults = async () => {
    try {
      const response = await studentResultsService.getPreviousResults(
        studentUserId
      );
      ...
      if (response.status === 200) {
        ...
        try {
          const allPreviousResultsandTerms = response?.data?.slice(1);
          console.log(
            "allPreviousResultsandTerms: ",
            allPreviousResultsandTerms
          );
          setResultsData(allPreviousResultsandTerms);
        } catch (error) {}
      } else {
  ...
  const renderTermResults = () => {
    return resultsData?.map((termResults, index) => (
      <div key={index} className="border-2 my-2 rounded-1">
        <Divider
          style={{
        ...
        <div className="row my-2">
          <div className="col-md-6">
            <Card hoverable>
              <Form.Item label="Academic Year">
                <Input
                  value={termResults && termResults[0]?.academic_year}
                  size={"large"}
                />
              </Form.Item>
            </Card>
          ...
          <div className="col-md-6 my-2">
            <Card hoverable>
              <Form.Item label="Academic Year">
                <Input
                  value={termResults && termResults[0]?.term_name}
                  size={"large"}
                />
              </Form.Item>
            </Card>
        ...
        <div className="col-12 rounded border-1">
          <div className="col-12">
            <Table
              className="table-responsive table-layout pb-2"
              dataSource={termResults?.slice(1)}
              columns={tableColumns}
              pagination={false}
              bordered={true}
              locale={{
                emptyText:
                  "Results Not Available. Please consult your teacher.",
              }}
        ...
        <div className="container">
          ...
          <img
            ...
            <div>
              ...
              <h5 className="text-center">STUDENT PREVIOUS RESULTS</h5>
            </div>
          </div>
          ...
          <div className="rounded py-3 mb-3">
            <div className="row mb-3">
              <Card>{renderTermResults()}</Card>
            </div>
