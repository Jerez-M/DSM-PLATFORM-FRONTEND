...
export async function AdminClassroomCourseworkPageLoader({ params }) {
    try {
...
const ViewAdminCourseworkPage = () => {
    ...
    const items = [
        ...
        {
            label: 'View Uploaded Marks',
            key: '3',
            onClick: () => navigate(`/admin/classroom/coursework/marks/${coursework?.id}`)
            
        },
    ];

    const fetchStudentCourseworkMark = async () => {
        const studentsResponse = await studentService.getStudentByUserId(authenticationService.getUserId())
        const student_id = studentsResponse?.data?.id
    ...
    return (
        <>
            ...
            >
                {studentCourseworkMark.length === 0 ? (
                    <Alert
                        message="Submit link not yet available, please consult the subject teacher."
                        type="info"
                        showIcon
                    />
                ) : (
...
export default ViewAdminCourseworkPage