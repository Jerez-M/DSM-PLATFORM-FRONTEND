...
export async function markCourseworkLoader({params}) {
    try {
            ...
            render: (record) => (
                <center>
                    <input
                        ...
                        type='number'
                        className='form-control form-control-sm me-0 test w-75'
                        // defaultValue={()=>{const hasMarkUploaded = studentsCourseworkMarks.some(
                        //     (mark) => mark.student?.id === record?.id
                        //   );
                    
                        //   return hasMarkUploaded ? (
                        //     hasMarkUploaded?.mark
                        //   ) : (
                        //     null
                        //   );}}
                        onBlur={
                            event => {
                                if (Number(event.target.value) > coursework?.total_mark) {
                                    event.target.value = coursework?.total_mark
                                }
            ...
            render: (record) => (
                <center>
                    <input
                        min={0}
                        max={coursework?.total_mark}
                        className='form-control form-control-sm me-0 test w-100'
                        onBlur={
                            event => {
                                for (let i = 0; i < results.length; i++) {
                                    if (results[i]['student'] === record?.id) {
                                        if (results[i]['comment'] !== event.target.value) {
                                            if (event.target.value === '') {
                                                results.splice(i, 1)
                                                return 0
                                            }
