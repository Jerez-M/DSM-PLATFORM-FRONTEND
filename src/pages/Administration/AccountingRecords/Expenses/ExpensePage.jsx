...
const ExpensePage = () => {
    ...
    useEffect(() => {
        if (activeTermId) {
            fetchExpenseByTermId(activeTermId);
        }
    }, [activeTermId]);
    ...
    const handleChangeTerm = (value) => {
        const term_id = value;
        fetchExpenseByTermId(term_id);
    };

    const fetchExpenseByTermId = async (term_id) => {
        try {
            const institution_id = authenticationService.getUserTenantId();

            const currentIncome = await accountingService.getExpensesByInstitutionIdAndTermId(
                institution_id,
                term_id
            );
            ...
            if (currentIncome?.status === 200) {
                ...
                message.error("No expense transactions found");
            }
        } catch (error) {
            message.error("No expense transactions found");
        }
